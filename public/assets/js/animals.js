...
const getAnimals = (formData = {}) => {
  ...
  /* when using fetch, you have to check that the "ok" property is true or false,
   and have to use .json to parse the material to readable format.
   printResults()- function is where it generates cards for each animal and prints to page*/
  fetch(queryUrl)
    .then(response => {
      if (!response.ok) {
